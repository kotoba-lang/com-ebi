(ns ebi.main
  "Kotodama WASM entrypoint for the Ebi clean-room actor (L4) — Clojure port.
  ...
  facts (`ebi.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ebi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ebi-compat" :tier tier :entities entities} 200])
